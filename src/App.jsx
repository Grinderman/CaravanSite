...
export default function HemsbyCaravanRental() {
  ...
  const photos = [
    { title: "Main Living Area", src: "/images/lounge.jpg", alt: "Main living area inside the caravan", className: "md:col-span-2 md:row-span-2" },
    { title: "Kitchen", src: "/images/kitchen.jpg", alt: "Kitchen inside the caravan" },
    { title: "Dining Area", src: "/images/dining.jpg", alt: "Dining area inside the caravan" },
    { title: "Bedroom", src: "/images/bedroom.jpg", alt: "Bedroom inside the caravan" },
    { title: "Shower Room", src: "/images/shower.jpg", alt: "Shower room inside the caravan" },
    { title: "Outdoor Area", src: "/images/outside.jpg", alt: "Outdoor area around the caravan" },
  ];
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-sky-900 via-sky-700 to-cyan-600 text-white">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-6 py-20 md:grid-cols-2 md:items-center lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <p className="mb-4 inline-flex rounded-full bg-white/15 px-4 py-2 text-sm font-medium backdrop-blur">Hemsby seaside caravan getaway</p>
            <h1 className="text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">Relax by the coast in a cosy, pet-friendly caravan</h1>
            <p className="mt-6 max-w-xl text-lg leading-8 text-sky-50">A welcoming caravan at Seafield Caravan Park, Hemsby — perfect for beach days, family breaks, and peaceful Norfolk getaways.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <a href="#booking" className="rounded-2xl bg-white px-6 py-3 text-center font-semibold text-sky-800 shadow-lg transition hover:bg-sky-50">Enquire About Dates</a>
              <a href="#gallery" className="rounded-2xl border border-white/50 px-6 py-3 text-center font-semibold text-white transition hover:bg-white/10">View Gallery</a>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.1 }} className="rounded-[2rem] bg-white/15 p-4 shadow-2xl backdrop-blur">
            <img src="/images/outside.jpg" alt="Caravan preview" className="aspect-[4/3] w-full rounded-[1.5rem] object-cover shadow-inner" />
          </motion.div>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-2 md:items-center lg:px-8">
        <div>
          ...
          <p className="mt-5 text-lg leading-8 text-slate-600">The caravan is based at Seafield Caravan Park on Newport Road, Hemsby. Before arrival, guests receive a map with the exact location, making it easy to find the caravan once on site.</p>
      ...
      <section id="booking" className="bg-slate-900 py-16 text-white">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 md:grid-cols-2 md:items-center lg:px-8">
          <div>
            ...
            <p className="mt-5 text-lg leading-8 text-slate-300">Get in touch to check availability, pricing, and any questions about your stay.</p>
          </div>
